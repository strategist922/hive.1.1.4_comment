 ...
 */
public class HadoopYuntiShims implements HadoopShims {
  ...
  public abstract static class CombineFileInputFormatShim<K, V> extends
    ...
    private List<MultiFileSplit> getSplitsForPath(JobConf job, Path path, int numSplits)
      ...
      LocatedFileStatus[] locatedFileStatus = fs.listLocatedStatus(path);
      Path[] paths = FileUtil.stat2Paths(locatedFileStatus);
      ...
      if (paths.length != 0) {
        ...
        for(int i=0; i<paths.length; i++) {
          if(locatedFileStatus[i].isDir()) {
            lengths[i] = fs.getContentSummary(paths[i]).getLength();
          } else {
            lengths[i] = locatedFileStatus[i].getLen();
          }
          totLength += lengths[i];
        }
        ...
        while (totalSize < lengths.length) {
          ...
          if (splitSize != 0) {
            ...
            LocatedFileStatus[] splitLocatedFileStatus = new LocatedFileStatus[splitSize];
            System.arraycopy(paths, startIndex, splitPaths , 0, splitSize);
            System.arraycopy(lengths, startIndex, splitLengths , 0, splitSize);
            System.arraycopy(locatedFileStatus, startIndex, splitLocatedFileStatus , 0, splitSize);
            splits.add(new MultiFileSplit(job, splitPaths, splitLengths, splitLocatedFileStatus));
  ...
  public static class InputSplitShim // extends MultiFileSplit
    ...
    public InputSplitShim() {
      delegate = new MultiFileSplit(null, new Path[0], new long[0], new LocatedFileStatus[0]);
    ...
    public InputSplitShim(JobConf job, MultiFileSplit old) throws IOException {
      delegate = new MultiFileSplit(job, old.getPaths(), old.getLengths(), old.getLocatedFileStatus());
   ...
   */
  public static class MultiFileSplit implements InputSplit {
    ...
    public MultiFileSplit(JobConf job, Path[] files, long[] lengths, LocatedFileStatus[] locatedFileStatus) {
      this.job = job;
    ...
    public String[] getLocations() throws IOException {
      ...
      for (int i=0 ; i < paths.length ; i++) {
        BlockLocation[] blkLocations = locatedFileStatus[i].getBlockLocations();
    ...
    public void readFields(DataInput in) throws IOException {
      ...
      int locatedFileStatusLength = in.readInt();
      locatedFileStatus = new LocatedFileStatus[locatedFileStatusLength];
      for(int i=0; i<locatedFileStatusLength; i++) {
        locatedFileStatus[i] = new LocatedFileStatus();
        locatedFileStatus[i].readFields(in);
      }
    }
    ...
    public void write(DataOutput out) throws IOException {
      ...
      out.writeInt(locatedFileStatus.length);
      for(LocatedFileStatus ls : locatedFileStatus) {
        ls.write(out);
      }
    }
  ...
  public void setNullOutputFormat(JobConf conf) {
    ...
    conf.setOutputCommitter(HadoopYuntiShims.NullOutputCommitter.class);
