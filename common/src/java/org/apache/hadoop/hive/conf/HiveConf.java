...
public class HiveConf extends Configuration {
  ...
  public static enum ConfVars {
    ...
    INTERMEDIATEJOB("hive.exec.intermediate.job", false),
    ...
    // if yes:
    // run in local mode only if input bytes is less than this. 128MB by default 低于输入byte数时候
    ...
    HIVEMAPJOINROWSIZE("hive.mapjoin.size.key", 10000),//HIVE-1754后已经移除该参数
    HIVEMAPJOINCACHEROWS("hive.mapjoin.cache.numrows", 25000),//HIVE-1754后已经移除该参数，原始是给jdbm的mapjoin使用
    HIVEGROUPBYMAPINTERVAL("hive.groupby.mapaggr.checkinterval", 100000),
    HIVEMAPAGGRHASHMEMORY("hive.map.aggr.hash.percentmemory", (float) 0.5),//map aggr内存比例，如果发生溢出，调低这个比例。因为估计得出的条数过大，单row过小
    ...
    HIVESMALLTABLESFILESIZE("hive.mapjoin.smalltable.filesize",15000000L), //15M
    ...
    HIVEMAXMAPJOINSIZE("hive.mapjoin.maxsize", 100000),//HIVE-1754后去除
    HIVELIMITMAXROWSIZE("hive.limit.row.max.size", 100000L),
    HIVELIMITOPTLIMITFILE("hive.limit.optimize.limit.file", 10),
    HIVELIMITOPTENABLE("hive.limit.optimize.enable", true),
    ...
    HIVEMAPJOINLOCALTASKTIMEOUT("hive.mapjoin.localtask.timeout", 60 * 1000),
    ...
    // Serde for FetchTask 修改这里能实现查询的打印结果
    ...
    // hive cli encoding
    HIVECLIENCODING("hive.cli.encoding", "UTF-8"),

    // log plan progress
    HIVELOGPLANPROGRESS("hive.log.plan.progress", true),

    // Wether pull counters from job tracker
    HIVEPULLPROGRESSCOUNTERS("hive.pull.progress.counters", true),

    // PreJob Hooks
    PREJOBHOOKS("hive.job.pre.hooks", ""),

    // PostJob Hooks
    POSTJOBHOOKS("hive.job.post.hooks", ""),

    HIVE_AUTHORIZATION_ENABLED("hive.security.authorization.enabled", false),

    HIVE_AUTHENTICATION_IMPL("hive.security.authentication.impl", ""),

    HIVE_AUTHORIZATION_IMPL("hive.security.authorization.impl", ""),

    // Max progress counters
    HIVEMAXPROGRESSCOUNTERS("hive.max.progress.counters", 100),

    // Check fatal errors interval
    HIVECHECKFATALERRORSINTERVAL("hive.check.fatal.errors.interval", 5000L),

    CLEANHOOKS("hive.clean.hooks", ""),

    HIVEMETADATAHOOKS("hive.metadata.hooks",""),
