...
import org.apache.hadoop.hive.service.SequentialHiveClientInvocationHandler;
...
public class HiveConnection implements java.sql.Connection {
  ...
  public HiveConnection(String uri, Properties info) throws SQLException  {
    ...
    // parse uri
    // If uri is not specified, use embedded mode.
    session.setUserName(info.getProperty("user"));
    session.setPassword(info.getProperty("password"));
    try {
      if (uri.isEmpty()) {
        client = new HiveServer.HiveServerHandler();
        if (!session.getAuthenticator().login(session)) {
        ...
        // Get sequential HiveClient
        InvocationHandler handler = new SequentialHiveClientInvocationHandler(protocol);
        client = (HiveInterface)Proxy.newProxyInstance(
            HiveInterface.class.getClassLoader(),
            new Class<?>[]{HiveInterface.class}, handler);

        if (!session.getAuthenticator().login(session, protocol)) {
          throw new SQLException("Login failed!");
        }
      }
  ...
  public HiveInterface getHiveInterface() {
    return client;
  }
