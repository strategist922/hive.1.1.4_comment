...
public class HiveDatabaseMetaData implements java.sql.DatabaseMetaData {
  ...
  public ResultSet getColumnPrivileges(String catalog, String schema,
    ...
    return new HiveMetaDataResultSet(new ArrayList(), new ArrayList(), null) {
      public boolean next(){
        return false;
      }
    };
    ...
    try {
          ...
          for (FieldSchema field: fields) {
            if (field.getName().matches(regcolumnNamePattern)) {
              columns.add(new JdbcColumn(field.getName(), table, catalog
                      , field.getType(), field.getComment(), ordinalPos, false));
              ordinalPos++;
            }
          }
          fields = client.get_table(catalog, table).getPartitionKeys();
          if (fields != null && !fields.isEmpty()) {
            for (FieldSchema field: fields) {
              if (field.getName().matches(regcolumnNamePattern)) {
                columns.add(new JdbcColumn(field.getName(), table, catalog
                        , field.getType(), field.getComment(), ordinalPos, true));
                ordinalPos++;
              }
            }
          }
      ...
      return new HiveMetaDataResultSet<JdbcColumn>(
              ...
              , Arrays.asList("STRING", "STRING", "STRING", "STRING", "I32", "STRING"
                ...
                , "STRING", "STRING", "STRING", "STRING", "I32", "STRING")
              , columns) {
