...
public class DataUtil
{
  ...
  protected static void init(Configuration conf) throws Exception {
    ...
    dbDriver = conf.get("taobao.meta.ConnectionDriverName", "");
    if ("".equals(dbDriver)) {
    ...
    dbUser = conf.get("taobao.meta.ConnectionUserName");
    if ("".equals(dbUser)) {
  ...
  public Map<Object, Object> getAverage(String md5, int timespan) throws NullException, ParseException
  {
    ...
    sql_from.append(" from ").append("hive_optimize").append(" where ").append("bizdate").append(" <= '").append(Function.format(bizdate, "yyyyMMdd", "yyyy-MM-dd")).append("' and ").append("bizdate").append(" > '").append(begin_date).append("' and ").append("md5sum").append("='").append(md5).append("'");

    if (skyId > 0) {
      sql_from.append(" and ").append("id").append("=").append(skyId);
    }
  ...
  public void setExclusive() {
    if (!this.conf.getBoolean("mapred.client.hooks.autored.exclusive", false))
      return;
    if ((isChecked) && (isExclusive))
      this.conf.setBoolean("mapred.client.hooks.autored.exclusive", false);
    try
    {
    ...
    this.conf.setBoolean("mapred.client.hooks.autored.exclusive", isExclusive);
  }
  ...
  public String getExclusiveSql()
  {
    ...
    String sql = new StringBuilder().append("select type,date_format(bizdate,'%Y%m%d') as bizdate from hive_optimize_exclusive where ").append(condition_1).append(" order by ").append("type").append(";").toString();
