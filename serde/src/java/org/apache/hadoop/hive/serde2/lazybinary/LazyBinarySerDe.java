  ...
  private void serializeStruct(Output byteStream, Object obj,
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (null != soi.getStructFieldData(obj, fields.get(i))) {
        nullByte |= 1 << (i % 8);//非空fields的位设置为1，空则为0
      }
      ...
      if (7 == i % 8 || i == size - 1) {//凑到8个，或者最后不足8个，为一组输出。输出一个字节nullbyte位图，然后输出8个field
  ...
  private void serialize(Output byteStream, Object obj,
    ...
    case PRIMITIVE: {
      ...
      case STRING: {
        ...
        LazyBinaryUtils.writeVInt(byteStream, length);//先写长度，再写byte数组
        /* write string itself */
