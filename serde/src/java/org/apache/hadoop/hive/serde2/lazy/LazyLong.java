...
public class LazyLong extends
  ...
  public static long parseLong(String i, int radix, long MAX_VALUE, long MIN_VALUE) {
    if (i == null) {
      throw new NumberFormatException("String is null");
    }
    ...
    String trimedStr = i.trim();    
    
    int strLength = trimedStr.length();
    ...
    if (trimedStr.startsWith("-")) {
      negtive = true;
      startIdx = 1;
    } else if (trimedStr.startsWith("+")) {
      startIdx = 1;
    }
    
    if (startIdx == 1 && strLength == 1) {
      throw new NumberFormatException(LazyUtils.convertToString(trimedStr.getBytes(), 0, strLength));
    }
    
    BigInteger result = BigInteger.ZERO;
    BigInteger lower = new BigInteger(Long.toString(MIN_VALUE, radix), radix).negate();
    BigInteger upper = new BigInteger(Long.toString(MAX_VALUE, radix), radix).add(lower);
    BigInteger radixBInt = new BigInteger(Long.toString(radix, radix), radix);

    // in mysql, for signed bigint,
    // if the number is equal or greater than 2^64-1 then cast returns -1;
    // if the number is equal or lesser than -2^63 then cast returns -2^63
    while (startIdx < strLength) {
      char thisCh = trimedStr.charAt(startIdx++);
      if (thisCh >= '0' && thisCh <= '9') { //decimal, not all
        result = result.multiply(radixBInt).add(new BigInteger(Long.toString(thisCh-'0', radix), radix));
        if (negtive) {
