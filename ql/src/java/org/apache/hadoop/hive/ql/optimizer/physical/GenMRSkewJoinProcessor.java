...
public final class GenMRSkewJoinProcessor {
  ...
  public static void processSkewJoin(JoinOperator joinOp,
    ...
    for (int i = 0; i < numAliases - 1; i++) {
      ...
      ByteArrayOutputStream xmlPlan = currPlan.toXML();
      ByteArrayInputStream bis;
      bis = new ByteArrayInputStream(xmlPlan.toByteArray());
  ...
  public static boolean skewJoinEnabled(HiveConf conf, JoinOperator joinOp) {
    ...
    if (!joinOp.getConf().isNoOuterJoin()) {// join skew 控制不支持outerjoin？
      return false;
    }

    byte pos = 0;
    for (Byte tag : joinOp.getConf().getTagOrder()) {// join skew 控制不支持乱序join？即STREAMTABLE
      if (tag != pos) {
        return false;
      }
      pos++;
