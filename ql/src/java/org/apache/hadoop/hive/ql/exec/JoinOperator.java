...
//row 的构成： joinkey +　value(lazystruct) + alias 的list
  @Override
  public void processOp(Object row, int tag) throws HiveException {
    try {
      ...
      if ((lastAlias == null) || (!lastAlias.equals(alias))) {//如果第一个输入源，或者换了输入源，则初始化emit的步进.lastAlias似乎一直null，没赋值
        nextSz = joinEmitInterval;//emit固定步进
      }
      ...
      ArrayList<Object> nr = JoinUtil.computeValues(row, joinValues.get(alias),
          ...
          joinFilterObjectInspectors.get(alias), noOuterJoin);//取出join后需取的value + 是否outterjoin

      if (handleSkewJoin) {
        skewJoinKeyContext.handleSkew(tag);
      }

      // number of rows for the key in the given table 当前存储的行数
      int sz = storage.get(alias).size();
      StructObjectInspector soi = (StructObjectInspector) inputObjInspectors[tag];
      StructField sf = soi.getStructFieldRef(Utilities.ReduceField.KEY
          .toString());
      Object keyObject = soi.getStructFieldData(row, sf);//这里struct的fieldid 即0.取得key

      // Are we consuming too much memory 如果是最后一个输入源。看上去是稀疏表在前，稠密表在后，能使用迭代器方法
      if (alias == numAliases - 1 && !(handleSkewJoin && skewJoinKeyContext.currBigKeyTag >= 0)) {//如果skew控制有作用目标了，那就不进行forward了，在endgroup进行复制。这时候顺序不重要了
        if (sz == joinEmitInterval) {//emit forword的步进到了。 当前存储的行数到达1000时候，执行join
          ...
          checkAndGenObject();//在加入最后表的一行记录前，执行join。
          storage.get(alias).clear();//只清理最后表容器的所有记录。这样后面的计算还是正确的
        }
      } else {//？为什么最后一个输入源emit时候不打log？这里是skew控制？
        if (sz == nextSz) {//打log的步进
      ...
      // Add the value to the vector 存入对应多row容器
      storage.get(alias).add(nr);
      // if join-key is null, process each row in different group.对于null的key进行一个一次join.这里是由于sql标准？null输出一条匹配
  ...
  public void endGroup() throws HiveException {
    ...
    if (handleSkewJoin && skewJoinKeyContext.currBigKeyTag >= 0) {//如果skew控制有作用目标了，那就不进行forward了，在endgroup进行复制
      try {
