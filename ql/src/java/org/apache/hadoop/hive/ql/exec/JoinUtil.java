...
public class JoinUtil {
  ...
  public static int populateJoinKeyValue(Map<Byte, List<ExprNodeEvaluator>> outMap,
    ...
    while (entryIter.hasNext()) {//每个输入源
      ...
      for (int j = 0; j < sz; j++) {
        if(key == (byte) posBigTableAlias){//该源是否为大表位置
          valueFields.add(null);
        }else{
          valueFields.add(ExprNodeEvaluatorFactory.get(expr.get(j)));//从desc中得到ExprNode求值器
        }
      }
  ...
  /**从lazy strutc的byte中解出对应表的select value列？返回writable对象
   * Return the value as a standard object. StandardObject can be inspected by a
   * standard ObjectInspector.
   */
  public static ArrayList<Object> computeValues(Object row,
    ...
    if (!noOuterJoin) {
      // add whether the row is filtered or not. 附带过滤条件求值后的结果。这里是join条件中的表达式。包括常量表达式？
