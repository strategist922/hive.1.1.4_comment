...
public final class PcrOpProcFactory {
  ...
  public static class FilterPCR implements NodeProcessor {
    ...
    public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx,
      ...
      if (fop2 != null && !fop2.getConf().getIsSamplingPred()) {// fop2只在sampling时候有用？
        return null;
      }
      ...
      PrunedPartitionList prunedPartList = owc.getParseContext().getOpToPartList().get(top);//已经优化过的
      if (prunedPartList == null) {
        ...
        try {
          ...
          if (prunedPartList != null) {
            owc.getParseContext().getOpToPartList().put(top, prunedPartList);//把三种分区true，denied，unkonow结果放入PcrOpWalkerCtx
          }
