...
/**
 * Base operator implementation.  T是关于操作符的Desc
 **/
public abstract class Operator<T extends Serializable> implements Serializable,
  ...
  /**
   * Is HIVEJOBPROGRESS turned on
   */
  protected boolean jobProgress = false;

  private static String groupName = "Hive Counters";
  ...
  public void initialize(Configuration hconf, ObjectInspector[] inputOIs)
    ...
    // Once initialized, hive max progress counters assigned
    totalNumCntrs =
      HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVEMAXPROGRESSCOUNTERS);
    ...
    if (childOperators != null) {
      ...
      for (int i = 0; i < childOperatorsArray.length; i++) {
        ...
        childOperatorsTag[i] = parentOperators.indexOf(this);//使用tablescan操作符的列表id
   ...
   *          The object representing the row.
   * @param tag
   *          The tag of the row usually means which parent this row comes from.//tag标识父操作符号即tablescanOp，也是来源表
   *          Rows with the same tag should have exactly the same rowInspector
   *          all the time.
   */
  public void process(Object row, int tag) throws HiveException {
    ...
    preProcessCounter();
    processOp(row, tag);//tag标识父操作符号即tablescanOp，也是来源表
    postProcessCounter();
  }
  ...
  /**
   * All counter stuff below this hive max progress counters
   */
  private static int totalNumCntrs = 100;
  ...
  protected void incrCounter(String name, long amount) {
    ...
    // FIXME List.contains() is not somehow efficient.
    if (!getCounterNames().contains(counterName)) {
      LOG.warn("Using too many counters. Increase the total number of counters for "
          + counterName);
    } else if (reporter != null) {
      reporter.incrCounter(groupName, counterName, amount);
    }
  }
  ...
  public void updateCounters(Counters ctrs) {
    ...
    for (String counter : getCounterNames()) {
      counters.put(counter, ctrs.getGroup(groupName).getCounter(counter));
    }
  ...
  public boolean checkFatalErrors(Counters ctrs, StringBuilder errMsg) {
    ...
    if (!getCounterNames().contains(counterName)) {
      ...
      long value = ctrs.getGroup(groupName).getCounter(counterName);
  ...
  public void assignCounterNameToEnum() {
    ...
    jobProgress = true;
    ArrayList<String> counterNames = getCounterNames();
    lastCounterUsed += counterNames.size();

    // Currently, only maximum number of 'totalNumCntrs' can be used. If you want to
    // add more counters, increase the number of 'totalNumCntrs'
    if (lastCounterUsed > totalNumCntrs) {
      int moreCounters = totalNumCntrs - lastCounterUsed;
      // pop last 'moreCounters' names, usually 'initializeCounters()' controls
      // the size of 'counterNames', following is no use.
      // However, 'Operator' class can be extended, so counterNames may be modified.
      for (int i = 0; i < moreCounters; i++) {
        counterNames.remove(counterNames.size() - 1);
      }
      LOG.warn("Using too many counters ("
          + moreCounters + "more). Increase the total number of counters");
      return;
    }
  }
  ...
  public void initializeCounters() {
    ...
    if (lastCounterUsed + 3 <= totalNumCntrs) {
    ...
    if (newCntrs != null && lastCounterUsed + newCntrs.size() <= totalNumCntrs) {
  ...
  public boolean getJobProgress() {
    return jobProgress;
  }

  public void setJobProgress(boolean jobProgress) {
