...
public class DefaultGraphWalker implements GraphWalker {
  ...
  protected Stack<Node> opStack;//缓存未访问的中间节点。先入后出压入
  private final List<Node> toWalk = new ArrayList<Node>();
  private final HashMap<Node, Object> retMap = new HashMap<Node, Object>();//key是已经访问过的节点
  ...
  /**
   * starting point for walking. 用towalk实现深度优先，先序
   * 
   * @throws SemanticException
   */
  ...
  public void walk(Node nd) throws SemanticException {
    ...
    // add children, self to the front of the queue in that order 把节点和子节点压栈towalk
    getToWalk().add(0, nd);
