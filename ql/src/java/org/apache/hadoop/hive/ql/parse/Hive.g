...
@after { msgs.pop(); }
    : KW_CREATE kwUSER userItem (COMMA userItem)*
...
dropUserStatement
@init { msgs.push("drop user statement"); }
@after { msgs.pop(); }
...
alterUserStatement
@init { msgs.push("alter user statement"); }
@after { msgs.pop(); }
    : KW_ALTER kwUSER Identifier KW_SET kwPASSWORD StringLiteral
    -> ^(TOK_SETPASSWORD Identifier StringLiteral)
    ;
...
grantStatement
@init { msgs.push("grant statement"); }
@after { msgs.pop(); }
    : 
    KW_GRANT privilegeList KW_ON grantRevokeTarget KW_TO userList (KW_WITH withOptionList)?
    -> ^(TOK_GRANT privilegeList grantRevokeTarget userList withOptionList?)
    ;

privilegeList
@init { msgs.push("privilege list"); }
@after { msgs.pop(); }
    : privilegeType (COMMA privilegeType)*  -> ^(TOK_PRIVLIST privilegeType+)
    ;

privilegeType
    : KW_ALL kwPRIVILEGES?     -> TOK_PRIVALL 
    | KW_SELECT                -> TOK_PRIVSEL
    | KW_INSERT                -> TOK_PRIVINS
    | KW_CREATE                -> TOK_PRIVCRT
    | KW_ALTER                 -> TOK_PRIVALT
    | KW_DROP                  -> TOK_PRIVDROP
    | KW_CREATE kwUSER         -> TOK_PRIVCREATEUSER
    | KW_GRANT KW_OPTION       -> TOK_PRIVGRANT
    | kwSUPER                  -> TOK_PRIVSUPER
    ;

userList
@init { msgs.push("user list"); }
@after { msgs.pop(); }
    : userItem (COMMA userItem)*  -> ^(TOK_USRLIST userItem+)
    ;

grantRevokeTarget
@init { msgs.push("grant/revoke target"); }
@after { msgs.pop(); }
    : STAR                                       -> ^(TOK_PRIV_DB)
    | STAR DOT STAR                              -> ^(TOK_PRIV_GLOBAL)
    | dbName=Identifier DOT STAR                 -> ^(TOK_PRIV_DB $dbName)
    | dbName=Identifier DOT tblName=Identifier   -> ^(TOK_PRIV_TABLE $dbName $tblName)
    | tblName=Identifier                         -> ^(TOK_PRIV_TABLE $tblName)
    ;

withOptionList
    : withOption (COMMA withOption)* -> ^(TOK_WITHOPTS withOption+)
    ;
...
revokeStatement
@init { msgs.push("revoke statement"); }
@after { msgs.pop(); }
    :
    KW_REVOKE privilegeList KW_ON grantRevokeTarget KW_FROM userList
    -> ^(TOK_REVOKE privilegeList grantRevokeTarget userList)
    ;

showGrantsStatement
@init { msgs.push("show grants statement"); }
@after { msgs.pop(); }
    : KW_SHOW kwGrants (KW_FOR Identifier)?
    -> ^(TOK_SHOWGRANTS Identifier?)
    ;
...
kwPASSWORD
    : {input.LT(1).getText().equalsIgnoreCase("PASSWORD")}? Identifier
    ;

kwSUPER
    : {input.LT(1).getText().equalsIgnoreCase("SUPER")}? Identifier
    ;

kwPRIVILEGES
    : {input.LT(1).getText().equalsIgnoreCase("PRIVILEGES")}? Identifier
    ;

kwGrants
    : {input.LT(1).getText().equalsIgnoreCase("GRANTS")}? Identifier
    ;
