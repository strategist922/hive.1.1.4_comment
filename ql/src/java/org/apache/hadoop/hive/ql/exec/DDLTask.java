...
public class DDLTask extends Task<DDLWork> implements Serializable {
  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      ...
      errorMessage = "Table " + e.getTableName() + " does not exist";
      return 1;
  ...
  private int showFunctions(ShowFunctionsDesc showFuncs) throws HiveException {
    ...
    try {
      ...
      while (iterFuncs.hasNext()) {
        // create a row per table name
        outStream.write(iterFuncs.next().getBytes("UTF-8"));
        outStream.write(terminator);
      }
      ((FSDataOutputStream) outStream).close();
      outStream = null;
  ...
  private int describeFunction(DescFunctionDesc descFunc) throws HiveException {
    ...
    try {
      ...
      if (desc != null) {
        outStream.write(desc.value().replace("_FUNC_", funcName).getBytes("UTF-8"));
        if (descFunc.isExtended()) {
          Set<String> synonyms = FunctionRegistry.getFunctionSynonyms(funcName);
          if (synonyms.size() > 0) {
            outStream.write(("\nSynonyms: " + join(synonyms, ", ")).getBytes("UTF-8"));
          }
          if (desc.extended().length() > 0) {
            outStream.write(("\n"
                + desc.extended().replace("_FUNC_", funcName)).getBytes("UTF-8"));
          }
        }
      } else {
        if (funcClass != null) {
          outStream.write(("There is no documentation for function '"
              + funcName + "'").getBytes("UTF-8"));
        } else {
          outStream.write(("Function '" + funcName + "' does not exist.").getBytes("UTF-8"));
        }
      }
  ...
  private int describeTable(Hive db, DescTableDesc descTbl) throws HiveException {
    ...
    try {
      ...
      if (tbl == null) {
        FileSystem fs = resFile.getFileSystem(conf);
        outStream = (DataOutput) fs.create(resFile);
    ...
    try {
	  ...
	  while (iterCols.hasNext()) {
		...
		outStream.write(col.getName().getBytes("UTF-8"));
		outStream.write(separator);
		outStream.write(col.getType().getBytes("UTF-8"));
		outStream.write(separator);
		outStream.write(col.getComment() == null ? new byte[0] : col.getComment().getBytes("UTF-8"));
		outStream.write(terminator);
	  }
        } else {
  ...
  private int dropTable(Hive db, DropTableDesc dropTbl) throws HiveException {
    ...
    Table tbl = null;
    try {
      tbl = db.getTable(db.getCurrentDatabase(), dropTbl.getTableName(), false);
    } catch (InvalidTableException e) {
    ...
    if (SessionState.get() != null) {
      ...
      AuthorizeEntry entry = new AuthorizeEntry(null, tbl, null,
          PrivilegeLevel.TABLE_LEVEL.getPrivileges(), PrivilegeLevel.TABLE_LEVEL);
      try {
        SessionState.get().getAuthorizer().revoke(null, userName, entry);
      ...
      try {
        if (userName != null) {
          AuthorizeEntry entry =
            new AuthorizeEntry(database, null, null,
  ...
  private int dropDatabase(Hive db, DropDatabaseDesc dropDb)
    ...
    try {
      ...
      // drop database is idempotent
    }
    if (database == null) {
      return dropDb.getIfExists() ? 0 : -1;
    }
    // Drop all privileges
    if (SessionState.get() != null) {
      String userName = SessionState.get().getUserName();
      AuthorizeEntry entry = new AuthorizeEntry(database, null, null,
          PrivilegeLevel.DATABASE_LEVEL.getPrivileges(), PrivilegeLevel.DATABASE_LEVEL);
      try {
        SessionState.get().getAuthorizer().revoke(null, userName, entry);
      } catch (AuthorizeException e) {
        throw new HiveException(e);
    ...
    db.dropDatabase(dbName, true, dropDb.getIfExists());
    db.setCurrentDatabase(MetaStoreUtils.DEFAULT_DATABASE_NAME);
    return 0;
  }
  ...
  private int setGenericTableAttributes(Table tbl) {
    if(conf.getBoolean("hive.set.default.table.owner.enabled", false)) {
      try {
        if (SessionState.get() != null) {
          DefaultTableOwner tableOwnerImpl = ReflectionUtils.newInstance(
              conf.getClass("hive.set.default.table.owner.impl",
                  UGITableOwner.class, DefaultTableOwner.class), conf);
          tableOwnerImpl.setDefaultTableOwner(SessionState.get() , conf);
          String userName = SessionState.get().getUgi();
          tbl.setOwner(userName);
          if(!conf.getBoolVar(ConfVars.HIVE_AUTHORIZATION_ENABLED)) {
            try {
              ...
              LOG.error("create user: " + stringifyException(e1));
              e1.printStackTrace();
            } catch (NoSuchAlgorithmException nsae) {
              LOG.error("create user: " + stringifyException(nsae));
            }
          }
        }
      } catch (IOException e) {
        LOG.error("Enable set default table owner, but failed !");
      }
    } else {
      if (SessionState.get() != null) {
