...
/**
 * ExecReducer.
 * 这里没有让MR去管理分组，而是采用了判断上一个key是否相等，内部管理
 */
...
// key为HiveKey, values为迭代器
  public void reduce(Object key, Iterator values, OutputCollector output,
    ...
    try {
      ...
      tag.set((byte) 0);//tag 初始化值0
      if (isTagged) {//如果输入源有打标记，则在key上取该标记
        // remove the tag
        int size = keyWritable.getSize() - 1;
        tag.set(keyWritable.get()[size]);//取最后一位为输入源标记
        keyWritable.setSize(size);
      }

      if (!keyWritable.equals(groupKey)) {//如果是新的组? 这里没有让MR去管理分组，而是采用了判断上一个key是否相等，内部管理
        // If a operator wants to do some work at the beginning of a group
        if (groupKey == null) { // the first group
          groupKey = new BytesWritable();
        } else {//如果不为null，说明要进行end 上一个group。进行join
        ...
        reducer.startGroup();//开启新的组
        reducer.setGroupKeyObject(keyObject);//把当前组key也记录给JoinOperator
      }
      // System.err.print(keyObject.toString());
      while (values.hasNext()) {//每一行下
        BytesWritable valueWritable = (BytesWritable) values.next();
        // System.err.print(who.getHo().toString());
        try {
          valueObject[tag.get()] = inputValueDeserializer[tag.get()]//反序列化 从bytew到lazy的ByteArrayRef
        ...
        row.clear();//key 和 value 加入一个列表。这里就是row的构成： joinkey +　value(lazystruct) + alias 的list
        row.add(keyObject);
        row.add(valueObject[tag.get()]);
        // The tag is not used any more, we should remove it.不再使用？错。 这里作为输入源表的标识。一个byte
        row.add(tag);
        ...
        try {
          reducer.process(row, tag.get());//处理一条数据
        } catch (Exception e) {
