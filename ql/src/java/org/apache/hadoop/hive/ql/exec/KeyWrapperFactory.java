  ...
  class ListKeyWrapper extends KeyWrapper {
    ...
    public void getNewKey(Object row, ObjectInspector rowInspector) throws HiveException {
      ...
      for (int i = 0; i < keyFields.length; i++) {
        keys[i]  = keyFields[i].evaluate(row);//groupkey + distinctkey
      }
    }
...
//单列做key的包装器
  class TextKeyWrapper extends KeyWrapper {
    ...
    @Override
    public void getNewKey(Object row, ObjectInspector rowInspector) throws HiveException {//根据求值器的配置，从select后input中选择key的那列。在reduce的时候没distinct那列
      // Compute the keys
