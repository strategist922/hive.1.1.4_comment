...
public class QB {
  ...
  public void rewriteViewToSubq(String alias, String viewName, String definer, QBExpr qbexpr) {
    alias = alias.toLowerCase();
    String tableName = aliasToTabs.remove(alias);
    assert (viewName.equals(tableName));
    aliasToSubq.put(alias, qbexpr);
    if (definer != null) {
      setViewDefiner(qbexpr, definer);
    }
  }

  private void setViewDefiner(QBExpr qbexpr, String definer) {
    if (qbexpr.getOpcode() == QBExpr.Opcode.NULLOP) {
      qbexpr.getQB().setViewExpandedBy(definer);
    } else {
      setViewDefiner(qbexpr.getQBExpr1(), definer);
      setViewDefiner(qbexpr.getQBExpr2(), definer);
    }
  }
  ...
  public void setViewExpandedBy(String viewExpandedBy) {
    this.viewExpandedBy = viewExpandedBy;
  }

  public String getViewExpandedBy() {
    return viewExpandedBy;
  }
