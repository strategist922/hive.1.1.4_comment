...
public class MapJoinOperator extends AbstractMapJoinOperator<MapJoinDesc> implements Serializable {
  ...
  transient int maxMapJoinSize;//已经移除
  ...
  private void loadHashTable() throws HiveException {
    boolean localMode = HiveConf.getVar(hconf, HiveConf.ConfVars.HADOOPJT).equals("local");//如果是jt是local，才从本地读取hashtable
    String tmpURI = null;
    ...
    try {
      if (localMode) {
        ...
        for (Map.Entry<Byte, HashMapWrapper<AbstractMapJoinKey, MapJoinObjectValue>> entry : mapJoinTables
          ...
          if( conf.getDumpFilePrefix() != null ) {
            dumpFilePrefix = conf.getDumpFilePrefix();
          }
          String filePath = Utilities.generatePath(tmpURI, dumpFilePrefix, pos, currentFileName);
        ...
        for (Map.Entry<Byte, HashMapWrapper<AbstractMapJoinKey, MapJoinObjectValue>> entry : mapJoinTables
          ...
          String suffix = Utilities.generateFileName(dumpFilePrefix, pos, currentFileName);
  ...
  @Override
  public void processOp(Object row, int tag) throws HiveException {//只传入大表的tag
