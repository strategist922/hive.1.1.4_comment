...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
  ...
  private final HashMap<Table, Set<String>> referredTableToUsers;
  private final List<Table> destTables;
  private QB qb;
  private ASTNode ast;
  ...
  public SemanticAnalyzer(HiveConf conf) throws SemanticException {
    ...
    referredTableToUsers = new HashMap<Table, Set<String>>();
    destTables = new ArrayList<Table>();
    destTableId = 1;
    uCtx = null;
  ...
  private void getMetaData(QBExpr qbexpr, String viewDefiner) throws SemanticException {
    if (qbexpr.getOpcode() == QBExpr.Opcode.NULLOP) {
      QB qb = qbexpr.getQB();
      if (viewDefiner != null && !viewDefiner.isEmpty()) {
        qb.setViewExpandedBy(viewDefiner);
      }
      getMetaData(qb);
    } else {
      getMetaData(qbexpr.getQBExpr1(), viewDefiner);
      getMetaData(qbexpr.getQBExpr2(), viewDefiner);
    }
  }
  ...
  public void getMetaData(QB qb) throws SemanticException {
    try {
      ...
      for (String alias : tabAliases) {
        ...
        // Add to referred tables, null user means current user.
        Set<String> users = referredTableToUsers.get(tab);
        if (users == null) {
          users = new HashSet<String>();
          referredTableToUsers.put(tab, users);
        }
        users.add(qb.getViewExpandedBy());
      ...
      for (String alias : qb.getSubqAliases()) {
        ...
        getMetaData(qbexpr, qb.getViewExpandedBy());
      }
  ...
  private void replaceViewReferenceWithDefinition(QB qb, Table tab,
    ...
    qb.rewriteViewToSubq(alias, tab_name, tab.getOwner(), qbexpr);
  }
  ...
  private Operator genFilterPlan(String dest, QB qb, Operator input)
    ...
    return genFilterPlan(qb, (ASTNode) whereExpr.getChild(0), input);// 传入where的条件表达式
  }
   ...
   */
  @SuppressWarnings("nls")
  private Operator genFilterPlan(QB qb, ASTNode condn, Operator input)//父操作符号
      throws SemanticException {
    ...
    for (String alias : aliases) {
      ...
      for (Map.Entry<String, ColumnInfo> entry : fMap.entrySet()) {
        ...
        ExprNodeColumnDesc expr = new ExprNodeColumnDesc(colInfo.getType(),
            name, colInfo.getTabAlias(), colInfo.getIsPartitionCol());
        col_list.add(expr);
  ...
  private Operator genGroupByPlanGroupByOperator1(QBParseInfo parseInfo,
    ...
    Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
        new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations,
            distPartAgg, groupByMemoryUsage, memoryThreshold), new RowSchema(
  ...
  private Operator genGroupByPlanReduceSinkOperator(QB qb, String dest,
      Operator inputOperatorInfo, int numReducers,
  ...
  private Operator genGroupByPlan1MR(String dest, QB qb, Operator input)
    ...
    Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb,
        dest, input, numReducers, false, false);
  ...
  private Operator genGroupByPlanMapAggr1MR(String dest, QB qb,
    ...
    Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb,
        dest, groupByOperatorInfo, numReducers, true, false);
  ...
  private Operator genGroupByPlanMapAggr2MR(String dest, QB qb,
    ...
    if (!optimizeMapAggrGroupBy(dest, qb)) {
      ...
      // ////// Generate ReduceSink Operator
      Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb,
          dest, groupByOperatorInfo, -1, true, true);
      ...
      // ////// Generate ReduceSink Operator
      Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb,
          dest, groupByOperatorInfo, 1, true, false);
  ...
  private Operator genBucketingSortingDest(String dest, Operator input, QB qb,
    ...
    if (enforceBucketing || enforceSorting) {
      ...
      input = genReduceSinkPlanForSortingBucketing(dest_tab, input, sortCols, partnCols,
          maxReducers);
    ...
    case QBMetaData.DEST_TABLE: {
      ...
      if (parts != null && parts.size() > 0) { // table is partitioned
        ...
        if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.DYNAMICPARTITIONING)) { // allow DP
          ...
          HiveConf.setBoolVar(conf, HiveConf.ConfVars.HIVECONVERTJOIN, false);
  ...
  private ArrayList<ExprNodeDesc> genConvertCol(String dest, QB qb, Table tab,
      TableDesc table_desc, Operator input,
    ...
    for (Integer posn : posns) {
      ...
      ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo, rowFields.get(posn)
          .getInternalName(),
                                                   rowFields.get(posn).getTabAlias(), rowFields
                                                       .get(posn).getIsPartitionCol());
  ...
  private Operator genBodyPlan(QB qb, Operator input) throws SemanticException {
    ...
    Operator curr = input;//curr 是当前操作符。用于递进
    ...
    for (Entry<String, Operator> e : aliasToOpInfo.entrySet()) {
      ...
      if (lateralViews != null) {
        ...
        for (ASTNode lateralViewTree : aliasToLateralViews.get(alias)) {
          ...
          RowResolver lvForwardRR = opParseCtx.get(op).getRowResolver();
  ...
  private void genMapRedTasks(QB qb) throws SemanticException {
    ...
    // Sort by alias depth before traversing operator DAG
    // to prevent NPE when generating chained map joins
    ArrayList<ArrayList<Operator<? extends Serializable>>> depthQueue =
        new ArrayList<ArrayList<Operator<? extends Serializable>>>();
    for (Map.Entry<String, Operator<? extends Serializable>> entry : topOps.entrySet()) {
      String alias = entry.getKey();
      int depth = alias == null ? 0 : entry.getKey().split(":").length - 1;
      ArrayList<Operator<? extends Serializable>> opSet = null;
      try {
        opSet = depthQueue.get(depth);
        if (opSet == null) {
          opSet = new ArrayList<Operator<? extends Serializable>>();
          depthQueue.set(depth, opSet);
        }
      } catch (IndexOutOfBoundsException e) {
        for (int i = depthQueue.size(); i < depth; i++) {
          depthQueue.add(null);
        }
        opSet = new ArrayList<Operator<? extends Serializable>>();
        depthQueue.add(opSet);
      }
      opSet.add(entry.getValue());
    }
    Collections.reverse(depthQueue);
    for (ArrayList<Operator<? extends Serializable>> opSet : depthQueue) {
      if (opSet != null) {
        topNodes.addAll(opSet);
      }
    }
  ...
  public void analyzeInternal(ASTNode ast) throws SemanticException {
    ...
    if (ast.getToken().getType() == HiveParser.TOK_CREATETABLE) {
      isCreateTable = true;
      // if it is not CTAS, we don't need to go further and just return 无select的建立表，直接退出语义分析
  ...
  private ASTNode analyzeCreateTable(ASTNode ast, QB qb)
    ...
    case CREATE_TABLE: // REGULAR CREATE TABLE DDL
      ...
      validateCreateTable(crtTblDesc);
      genAuthorizeEntry(db.getCurrentDatabase(), null, null, Privilege.CREATE_PRIV);
      authorizeEntry.addDdlDesc(crtTblDesc);
    ...
    case CTLT: // create table like <tbl_name>
      ...
      genAuthorizeEntry(db.getCurrentDatabase(), null, null, Privilege.CREATE_PRIV);
      authorizeEntry.addDdlDesc(crtTblLikeDesc);
      rootTasks.add(TaskFactory.get(new DDLWork(getInputs(), getOutputs(),
          crtTblLikeDesc), conf));
      break;
  ...
  private ASTNode analyzeCreateView(ASTNode ast, QB qb)
    ...
    rootTasks.add(TaskFactory.get(new DDLWork(getInputs(), getOutputs(),
        createVwDesc), conf));
    genAuthorizeEntry(db.getCurrentDatabase(), null, null, Privilege.CREATE_PRIV);
    authorizeEntry.addDdlDesc(createVwDesc);
    return selectStmt;
  }
  ...
  @Override
  public void authorize(Authorizer authorizer, String userName) throws AuthorizeException {
    // if is create table statement
    if (isCreateTable) {
      authorizer.authorize(userName, authorizeEntry);
    }

    // if is query, CTAS is both a query and create table
    // Dest tables
    for (Table table : destTables) {
      authorizeEntry = new AuthorizeEntry(null, table, null, Privilege.INSERT_PRIV,
          PrivilegeLevel.TABLE_LEVEL);
      authorizer.authorize(userName, authorizeEntry);
    }

    List<AuthorizeEntry> entrys = new LinkedList<AuthorizeEntry>();
    // Source tables (not include views and intermediate tables)
    for (Entry<TableScanOperator, Table> e : topToTable.entrySet()) {
      Table tbl = e.getValue();
      TableScanOperator tsop = e.getKey();
      List<FieldSchema> neededFields = null;
      List<FieldSchema> fields = tbl.getCols();

      for (String user : referredTableToUsers.get(tbl)) {
        if (tsop.getNeededColumnIDs() != null) {
          for (Integer i : e.getKey().getNeededColumnIDs()) {
            if (neededFields == null) {
              neededFields = new ArrayList<FieldSchema>();
            }
            if (i < fields.size()) {
              neededFields.add(fields.get(i));
            } else {
              // virtual/partition fields
              neededFields.add(tbl.getPartCols().get(i - fields.size()));
            }
          }
        }

        authorizeEntry = new AuthorizeEntry(null, tbl, neededFields, Privilege.SELECT_PRIV,
            PrivilegeLevel.COLUMN_LEVEL);
        entrys.add(authorizeEntry);
      }
    }
    authorizer.authorize(userName, entrys);

    // views
    for (Table t : referredTableToUsers.keySet()) {
      if (t.isView()) {
        genAuthorizeEntry(t.getDbName(), t.getTableName(), null, Privilege.SELECT_PRIV);
        authorizer.authorize(userName, authorizeEntry);
      }
    }
  }
  ...
  private void decideExecMode(List<Task<? extends Serializable>> rootTasks, Context ctx,
    ...
    for (ExecDriver mrtask : mrtasks) {
      if(mrtask.getNonLimitOptimizationLocalModeTry()) {
        continue ;
      }
      try {
        ...
        if (MapRedTask.isEligibleForLocalMode(conf, numReducers, estimatedInput,
          ...
          if ( !(globalLimitCtx != null && globalLimitCtx.isEnable()
            ...
            mrtask.setNonLimitOptimizationLocalModeTry(true);
          }
        }
      } catch (IOException e) {
        throw new SemanticException(e);
    ...
    if (!hasNonLocalJob) {
      ...
      ctx.backupOriginalMasters(conf.getVar(HiveConf.ConfVars.HADOOPJT),
          conf.getVar(HiveConf.ConfVars.HADOOPNN));
      conf.setVar(HiveConf.ConfVars.HADOOPJT, "local");
      conf.setVar(HiveConf.ConfVars.HADOOPNN, "file:///");
