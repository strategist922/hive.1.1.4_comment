...
public class ExplainTask extends Task<ExplainWork> implements Serializable {
  ...
  private void outputPlan(Serializable work, PrintStream out, boolean extended,
    ...
    Arrays.sort(methods, new MethodComparator());
    													//            还有   MapredWork	，	 JoinDesc， SelectDesc，ExprNodeColumnDesc，ExprNodeDesc，FileSinkDesc，TableDesc，FetchWork
    for (Method m : methods) {//对MapredWork的方法反射。获取什么数据呢？MapredWork.getAliasToWork（） 返回根operator：TableScanOperator（带子op）
      ...
      if (note instanceof Explain) {
        ...
        if (extended || xpl_note.normalExplain()) {//如果该方法有explain的annotation
  ...
  private void outputDependencies(Task<? extends Serializable> task,
    ...
    if (dependeciesTaskSet.contains(task)) {//是否已访问过的task
      return;
    }
