...
// 主要的动作： 将skew的这一组group输出到临时文件中
  void endGroup() throws IOException, HiveException {
    if (skewKeyInCurrentGroup) {
      ...
      bigKey.copyToDFSDirecory(destFs, outputPath);//复制一个大表
      ...
      for (int i = 0; i < numAliases; i++) {
        ...
        if (values != null) {
          specPath = conf.getSmallKeysDirMap().get((byte) currBigKeyTag).get(
              (byte) i);
          values.copyToDFSDirecory(destFs, getOperatorOutputPath(specPath));//复制其他小表
        }
      }
    }
...
//主要记录了哪个中间表缓存倾斜了
  public void handleSkew(int tag) throws HiveException {
    ...
    if (currBigKeyTag == -1 && (tag < numAliases - 1)
      ...
      // a skew key now.
      currBigKeyTag = tag;// 标记最大的一个大表tag。这逻辑是最后一个超出标准的表
      ...
      skewKeyInCurrentGroup = true;// 标记是否倾斜了
      bigKeysExistingMap.put(Byte.valueOf((byte) currBigKeyTag), Boolean.TRUE);//记录了哪个中间表缓存倾斜了
    }
  }
