...
struct TQueryInfo {
  1: map<string, TTaskInfo> taskInfos,
  2: map<string, string> queryInfos
}

struct TCommandProcessorResponse {
  1: string message
  2: i32 responseCode
  3: string SQLState
  4: bool ready
...
service ThriftHive extends hive_metastore.ThriftHiveMetastore {
  ...
  # Get query info
  TQueryInfo getHiveQueryInfo() throws(1:HiveServerException ex)

  # Kill running jobs
  void killJob() throws(1:HiveServerException ex)

  # Execute statement asynchronizely
  void start(1:string query) throws(1:HiveServerException ex)

  # Wait for execution finish for asynchroinze submission
  TCommandProcessorResponse getResponse()

  # clean up last Hive query
  void clean()
}
