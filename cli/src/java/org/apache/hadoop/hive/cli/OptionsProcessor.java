...
public class OptionsProcessor {
  ...
  private final Option userOption, passwordOption;
  ...
  public OptionsProcessor() {
    ...
    // -u
    userOption = createOptionWithArg(builder, "user", "u", "user name",
        argBuilder.withMinimum(1).withMaximum(1).create());

    // -p
    passwordOption = createOptionWithArg(builder, "password", "p", "password for user",
        argBuilder.withMinimum(0).withMaximum(1).create());
  ...
  public boolean process_stage2(CliSessionState ss) {
    ss.getConf();
    // -u
    boolean hasUserName = cmdLine.hasOption(userOption);
    String userName = (String) cmdLine.getValue(userOption);
    if (hasUserName) {
      if (userName == null) {
        printUsage("User name is not valid. ");
        return false;
      } else {
        ss.setUserName((String) cmdLine.getValue(userOption));
      }
    }
    // -p
    boolean hasPwd = cmdLine.hasOption(passwordOption);
    String pwd = hasPwd ? (String) cmdLine.getValue(passwordOption) : "";
    if (!hasPwd && hasUserName) {
      try {
        ConsoleReader reader = new ConsoleReader();
        String line = reader.readLine("Enter password: ", '\0');
        pwd = line.trim();
  ...
  public void printUsage(String error) {
    ...
    System.err.println("");
    System.err
        .println("Usage: hive [--config confdir] [-hiveconf x=y]* [-i <init-filename>]*"
